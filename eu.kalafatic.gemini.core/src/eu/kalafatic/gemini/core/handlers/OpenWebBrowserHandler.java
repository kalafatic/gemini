...
/**
 * The Class class OpenWebBrowserHandler.
...
public class OpenWebBrowserHandler extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		String homeAddress = PREFERENCES.get(ECorePreferences.HOME_SITE.getName(), (String) ECorePreferences.HOME_SITE.getDef());

		IWorkbenchWindow window = PlatformUI.getWorkbench().getActiveWorkbenchWindow();

		IWorkbenchBrowserSupport support = window.getWorkbench().getBrowserSupport();
		try {
			support.getExternalBrowser().openURL(new URL(homeAddress));
		} catch (Exception e) {
