...
import eu.kalafatic.gemini.core.utils.NetstatUtils;
...
public class PortExplorerPage extends APreferencePage {
	...
	protected Control createContents(Composite parent) {
		try {
			...
			Group group = factory.createGroup(container, "Gemini ports", 2);
			createGEPortsBox(group);

			group = factory.createGroup(container, "Discover ports", 2);
	...
	private void createDiscoverPortsBox(final Group group) throws Exception {
		discoverPortText = factory.createText(group, "Set port number", true);

		discoverPortBtn = factory.createButton(group, "Discover", SWT.PUSH, this);

		discoverPortTable = factory.createTable(group, TAB_STYLE_2, "", true, true);
		((GridData) discoverPortTable.getLayoutData()).heightHint = 100;

		for (int i = 0; i < tableColumns.length; i++) {
			factory.createTableColumn(discoverPortTable, SWT.LEFT, tableColumns[i]).pack();
		}
	...
	private void createPortUsersTable(Table table, String[] split) {
		...
		for (int i = 1, row = 0; i < split.length / 4;) {
			factory.createTableItem(table, false, Integer.toString(row++), split[i++], split[i++], split[i++], split[i++], split[i++]);
	...
	private void createWebUpldPortBox(Group group) {
		...
		webUpldPortText = factory.createText(group, ECorePreferences.WEB_UPLOAD_PORT.getName(), true);
	...
	private void createUpdatePortBox(Group group) {
		...
		updatePortText = factory.createText(group, ECorePreferences.UPDATE_PORT.getName(), true);
	...
	private void createTrackerUpldPortBox(Group group) {
		...
		trackerUpldPortText = factory.createText(group, ECorePreferences.TRACKER_UPLOAD_PORT.getName(), true);
	...
	private void createTrackerPortBox(Group group) {
		...
		trackerPortText = factory.createText(group, ECorePreferences.TRACKER_PORT.getName(), true);
	...
	private void createBTPortBox(Group group) {
		...
		btPortText = factory.createText(group, ECorePreferences.BT_CLIENT_PORT.getName(), true);
	...
	private void createRCPortBox(Group group) {
		...
		rcPortText = factory.createText(group, ECorePreferences.RC_PORT.getName(), true);
	...
	private void createSingletonArgumentsPortBox(final Group group) {
		factory.createLabel(group, ECorePreferences.SINGLETON_ARGS_PORT.getName());

		singletonPortText = factory.createText(group, ECorePreferences.SINGLETON_ARGS_PORT.getName(), true);
	...
	protected void performApply() {
		try {
			PREFERENCES.putInt(ECorePreferences.SINGLETON_ARGS_PORT.getName(), Integer.parseInt(singletonPortText.getText()));
			PREFERENCES.putInt(ECorePreferences.RC_PORT.getName(), Integer.parseInt(rcPortText.getText()));
			PREFERENCES.putInt(ECorePreferences.BT_CLIENT_PORT.getName(), Integer.parseInt(btPortText.getText()));
			PREFERENCES.putInt(ECorePreferences.TRACKER_PORT.getName(), Integer.parseInt(trackerPortText.getText()));
			PREFERENCES.putInt(ECorePreferences.UPDATE_PORT.getName(), Integer.parseInt(updatePortText.getText()));
			PREFERENCES.putInt(ECorePreferences.WEB_UPLOAD_PORT.getName(), Integer.parseInt(webUpldPortText.getText()));
		} catch (Exception e) {
			DialogUtils.INSTANCE.showException(e);
		}
	}
