...
public class KeyAssistDialog extends PopupDialog {
	 ...
	 * @param associatedKeyboard the associated keyboard
	 * @param associatedState the associated state
	 */

	public KeyAssistDialog(final IWorkbench workbench, final WorkbenchKeyboard associatedKeyboard, final KeyBindingState associatedState) {
		super((Shell) null, PopupDialog.INFOPOPUP_SHELLSTYLE, true, false, false, true, true, "", "");

		this.activityManager = workbench.getActivitySupport().getActivityManager();
		this.bindingService = workbench.getService(IBindingService.class);
		this.commandService = workbench.getService(ICommandService.class);
	...
	private String getKeySequenceString() {
		...
		final TriggerSequence[] keyBindings = bindingService.getActiveBindingsFor(new ParameterizedCommand(command, null));
		...
		for (int i = 0; i < keyBindingsCount; i++) {
			...
			if (prefixSize > 0) {
				if (keySequence.startsWith(currentState, false)) {
					/*
					 * Okay, so we have a partial match. Replace the key binding with the required suffix completion.
					 */
...
//		noMatchesLabel.setText(KeyAssistMessages.NoMatches_Message);
	 ...
	 * @param parent the parent
	 * @param partialMatches the partial matches
	 */
	private final void createTableDialogArea(final Composite parent, final SortedMap<Binding, String> partialMatches) {
		// Layout the table.
	...
	private final void editKeyBinding() {
		...
		final PreferenceDialog dialog = PreferencesUtil.createPreferenceDialogOn(getShell(), keysPageId, null, binding);
	...
	private final SortedMap<Binding, String> getPartialMatches() {
		...
		final SortedMap<Binding, String> sortedMatches = new TreeMap<Binding, String>(new Comparator<Object>() {
			@Override
		...
		/*
		 * Remove those partial matches for which either the command is not identified or the activity manager believes the command is not enabled.
		 */
	...
	public final int open(Collection<?> bindings) {
		conflictMatches = new TreeMap<Binding, String>(new Comparator<Object>() {
			@Override
			public final int compare(final Object a, final Object b) {
				final Binding bindingA = (Binding) a;
				final Binding bindingB = (Binding) b;
	...
	private final void registerShellType() {
		final Shell shell = getShell();
		final IContextService contextService = keyBindingState.getAssociatedWindow().getWorkbench().getService(IContextService.class);
	...
	/*
	 * (non-Javadoc)
	 * @see org.eclipse.jface.window.Window#setParentShell(org.eclipse.swt.widgets .Shell)
	 */
	@Override
