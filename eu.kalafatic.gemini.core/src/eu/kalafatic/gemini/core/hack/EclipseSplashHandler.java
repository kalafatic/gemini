...
public class EclipseSplashHandler extends BasicSplashHandler {
	...
	public void init(Shell splash) {
		...
		progressRect = StringConverter.asRectangle(progressRectString, new Rectangle(10, 10, 300, 15));
		setProgressRect(progressRect);

		messageRect = StringConverter.asRectangle(messageRectString, new Rectangle(10, 35, 300, 15));
		...
		RGB rgb = new RGB((foregroundColorInteger & 0xFF0000) >> 16, (foregroundColorInteger & 0xFF00) >> 8, foregroundColorInteger & 0xFF);
...
//				int x = getBounds().width - (product.length() * 2 + 45);
				int x =10;
				int y = getBounds().height - 50;

				@Override
				public void paintControl(PaintEvent e) {
					e.gc.setForeground(foreground);
					e.gc.drawText(product+" v " + version, x, y, true);
				}
			});
		} catch (Exception e) {
			e.printStackTrace();
