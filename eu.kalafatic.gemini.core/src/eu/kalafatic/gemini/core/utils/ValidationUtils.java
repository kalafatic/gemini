...
import static eu.kalafatic.gemini.core.lib.constants.FConstants.DYNAMIC_PORTS;
import static eu.kalafatic.gemini.core.lib.constants.FConstants.REGISTERED_PORTS;
import static eu.kalafatic.gemini.core.lib.constants.FConstants.WELL_KNOWN_PORTS;
			...
			public IStatus validate(Object value) {
				...
				try {
					...
					if (text.trim().length() != 0) {
						...
						if (parseInt < (Integer) WELL_KNOWN_PORTS[2]) {
							status = ValidationStatus.warning((String) WELL_KNOWN_PORTS[0]);
						} else if (parseInt < (Integer) REGISTERED_PORTS[2]) {
							status = ValidationStatus.info((String) REGISTERED_PORTS[0]);
						} else if (parseInt < (Integer) DYNAMIC_PORTS[2]) {
							status = ValidationStatus.cancel((String) DYNAMIC_PORTS[0]);
						}
					}
