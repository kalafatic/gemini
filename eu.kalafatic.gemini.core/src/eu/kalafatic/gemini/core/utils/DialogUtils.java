...
public class DialogUtils {
	...
	public void showMessage(final int style, final String title, final String message) {
		...
		Display.getDefault().asyncExec(new Runnable() {
			...
			public void run() {
				try {
					MessageDialog.open(style, Display.getDefault().getActiveShell(), title, message, SWT.SHEET);
				} catch (Exception e) {
					Log.log(ECorePreferences.MODULE, e);
				}
			}
	...
	public void showException(final Exception e, final String title, final int level) {
		Display.getDefault().asyncExec(new Runnable() {
			...
			public void run() {
				try {
					MessageDialog.openWarning(Display.getDefault().getActiveShell(), title, getTrace(e, level));
				} catch (Exception e) {
					Log.log(ECorePreferences.MODULE, e);
				}
			}
