...
import static eu.kalafatic.gemini.core.lib.constants.FConstants.CERT_LOC;
...
public class SSLUtils {
	...
	public KeyStore loadKeystore() throws Exception {
		...
		String workspaceCertPath = PREFERENCES.get(ECorePreferences.CERT_LOC.getName(), (String) ECorePreferences.CERT_LOC.getDef());

		String appCertPath = CERT_LOC.concat(File.separator).concat("gemini.jks");
		...
		try {
			...
			if (PREFERENCES.getBoolean(ECorePreferences.CERT_ENABLED.getName(), (Boolean) ECorePreferences.CERT_ENABLED.getDef())) {
				return keyStore;
			}
			...
			PREFERENCES.putBoolean(ECorePreferences.CERT_ENABLED.getName(), true);
			PREFERENCES.flush();
