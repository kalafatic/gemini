...
/**
 * The Class class StopTorrentsHandler.
 * @author Petr Kalafatic
 * @project Gemini
 * @version 3.0.0
 */
public class StopTorrentsHandler extends ATorrentActionHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (activePart instanceof ViewPart) {
			...
			if (viewPart instanceof IViewer) {
				...
				if (tableViewer != null) {
					List<ExtTorrent> selected = getSelectedTorrents(tableViewer, false, EViewsMessages.STOPPED);
	...
	private void setCommandSourceForAdapter(TableViewer tableViewer, boolean all) {
		if (all) {
			...
			tableViewer.getTable().setBackground(FUIConstants.LIGHT_RED);
		} else {
