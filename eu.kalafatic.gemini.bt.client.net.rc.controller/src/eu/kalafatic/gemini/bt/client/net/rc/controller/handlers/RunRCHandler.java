/*******************************************************************************
 * Copyright (c) 2010, Petr Kalafatic (gemini@kalafatic.eu).
 * All rights reserved. This program and the accompanying materials
 * are made available under the terms of the GNU GPL Version 3
 * which accompanies this distribution, and is available at
 * http://www.gnu.org/licenses/gpl.txt
 *
 * Contributors:
 *     Petr Kalafatic - initial API and implementation
 ******************************************************************************/
package eu.kalafatic.gemini.bt.client.net.rc.controller.handlers;

import static eu.kalafatic.gemini.core.lib.constants.FConstants.PREFERENCES;

import org.eclipse.core.commands.AbstractHandler;
import org.eclipse.core.commands.ExecutionEvent;
import org.eclipse.core.commands.ExecutionException;

import eu.kalafatic.gemini.bt.client.net.rc.controller.model.RCModelManager;
import eu.kalafatic.gemini.bt.client.net.rc.controller.threads.RCServerThread;
import eu.kalafatic.gemini.core.lib.ECorePreferences;

/**
 * The Class class RunRCHandler.
 * @author Petr Kalafatic
 * @project Gemini
 * @version 3.0.0
 */
public class RunRCHandler extends AbstractHandler {

	/*
	 * (non-Javadoc)
	 * @see org.eclipse.core.commands.AbstractHandler#execute(org.eclipse.core.commands .ExecutionEvent)
	 */
	@Override
	public Object execute(ExecutionEvent event) throws ExecutionException {
		boolean run = PREFERENCES.getBoolean(ECorePreferences.RC_ENABLED.getName(), ((Boolean) ECorePreferences.RC_ENABLED.getDef()));

		if (run) {
			RCModelManager.getInstance();
			RCServerThread.getInstance().start();
		}
		return null;
	}
}
