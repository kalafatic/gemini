...
import static eu.kalafatic.gemini.core.lib.constants.FUIConstants.SAND_COLOR;
...
import eu.kalafatic.gemini.bt.client.net.controller.adapters.SwarmTreeAdapter;
...
import eu.kalafatic.gemini.bt.client.net.controller.providers.SwarmTreeContentProvider;
import eu.kalafatic.gemini.bt.client.net.controller.providers.SwarmTreeLabelProvider;
import eu.kalafatic.gemini.bt.client.net.controller.providers.SwarmTreeNameSorter;
...
/**
 * The Class class SwarmTreeView.
 * @author Petr Kalafatic
 * @project Gemini
 * @version 3.0.0
 */
public class SwarmTreeView extends AViewer {
	...
	/** The ROO t_ node. */
	private final String ROOT_NODE = "Swarms";
	...
	private void createContents(Composite parent) {
		...
		viewer.setContentProvider(new SwarmTreeContentProvider());
		viewer.setLabelProvider(new SwarmTreeLabelProvider(viewer));
		viewer.setSorter(new SwarmTreeNameSorter());
	...
	public void initAdapters(Collection<?> collection) {
		...
		for (ExtTorrent extTorrent : torrents.getFinishedTorrentsMap().values()) {
			extTorrent.eAdapters().add(swarmTreeAdapter);
		}
	}
	...
	public void hookContextMenu() {
		...
		menuMgr.addMenuListener(new IMenuListener() {
			...
			public void menuAboutToShow(IMenuManager manager) {
				SwarmTreeView.this.fillContextMenu(manager);
			}
		});
		...
		try {
			if (selection instanceof StructuredSelection) {
				...
				if (structuredSelection.getFirstElement() instanceof Entry<?, ?>) {
					selectionChanged((Entry<?, ?>) structuredSelection.getFirstElement());
				} else if (structuredSelection.getFirstElement() instanceof Session) {
					selectionChanged((Session) structuredSelection.getFirstElement());
				}
				updateSwarms();
			}
		} catch (Exception e) {
		...
		for (int i = 0; i < items.length; i++) {
			...
			if (items[i].getData() instanceof Entry) {
				...
				if (entry.getValue() instanceof SwarmSession) {
					...
					ExtTorrent extTorrent = NetworkModelManager.getInstance().getDwnldUpldExtTorrent(session.getAnnounce());
