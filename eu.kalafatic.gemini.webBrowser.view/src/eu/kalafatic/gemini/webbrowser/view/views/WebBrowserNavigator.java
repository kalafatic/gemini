 ...
 ******************************************************************************/
package eu.kalafatic.gemini.webbrowser.view.views;
...
import eu.kalafatic.gemini.webbrowser.controller.model.WebModelManager;
import eu.kalafatic.gemini.webbrowser.controller.providers.WebNavigatorContentProvider;
import eu.kalafatic.gemini.webbrowser.controller.providers.WebNavigatorLabelProvider;

/**
 * The Class class WebBrowserNavigator.
 * @author Petr Kalafatic
 * @project Gemini
 * @version 3.0.0
 */
public class WebBrowserNavigator extends ViewPart {
	...
	/**
	 * Instantiates a new web browser navigator.
	 */
	...
	private void createContents(Composite parent) {
		...
		EMap<String, Folder> inputMap = WebModelManager.getInstance().getWeb().getFolders();

		viewer.setContentProvider(new WebNavigatorContentProvider(inputMap));
		viewer.setLabelProvider(new WebNavigatorLabelProvider(inputMap));
	...
	private void hookContextMenu() {
		...
		menuMgr.addMenuListener(new IMenuListener() {
			...
			public void menuAboutToShow(IMenuManager manager) {
				WebBrowserNavigator.this.fillContextMenu(manager);
			}
		});
	...
	private void makeActions() {
		...
		addPageAction = new Action() {
			@Override
			public void run() {
				WebBrowserView findView = (WebBrowserView) PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().findView(EView.BROWSER.ID);
				...
				if (selection instanceof TreeSelection) {
					...
					if (treeSelection.getFirstElement() instanceof FolderImpl) {
						...
						WebModelManager.getInstance().doSave();
						refresh();
					}
				}
			}
		...
		removeAction = new Action() {
			...
			public void run() {
				...
				if (selection instanceof TreeSelection) {
					...
					if (treeSelection.getFirstElement() instanceof FolderImpl) {
						FolderImpl folder = (FolderImpl) treeSelection.getFirstElement();
						WebModelManager.getInstance().getWeb().getFolders().removeKey(folder.getAddress());

					} else if (treeSelection.getFirstElement() instanceof PageImpl) {
						TreePath[] paths = treeSelection.getPaths();
						viewer.remove(paths[paths.length - 1]);
					}
					Display.getDefault().asyncExec(new Runnable() {
						@Override
						public void run() {
							WebModelManager.getInstance().doSave();
							viewer.getTree().getShell().layout(true, true);
							refresh();
						}
					});
	...
	private void addFolder() {
		...
		if (selection instanceof TreeSelection) {
			...
			if (dialog.open() == Window.OK) {
				...
				WebModelManager.getInstance().getWeb().getFolders().put(folderName, folder);
