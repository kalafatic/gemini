 ...
 * @project Gemini
 */
public class SyncSummaryPage extends ATrackerSyncPage {

	/** The swarm clients. */
	public Set<String> swarmClients = new HashSet<String>();
	 ...
	 */
	public SyncSummaryPage(GUIFactory factory, ATrackerSyncWizard wizard) {
	...
	private void createDestTables() {
		...
		for (Table table : wizard.dest.values()) {
			...
			if (wizard.export && tableName.equals(ECorePreferences.SYNC_DB.getName())) {
				clonedTable = wizard.createTable(group, tableName, "Category", "Name", "Note");
				dbExport = true;

			} else {
				clonedTable = wizard.createTable(group, tableName, "Name", "Note");
			}
	...
	private void addClient() {
		try {
			...
			EditClientDialog editClientDialog = new EditClientDialog(getShell(), request);
			...
			if (editClientDialog.open() == Window.OK) {
				...
				if (host.length() == 0 || port.length() == 0) {
					MessageDialog.openInformation(getShell(), "Client", "Can not read address");
					return;
				}
				...
				if (uniqueIP(ip)) {
					...
					MessageDialog.openInformation(getShell(), "Client", "Duplicate host");
				}
			}
