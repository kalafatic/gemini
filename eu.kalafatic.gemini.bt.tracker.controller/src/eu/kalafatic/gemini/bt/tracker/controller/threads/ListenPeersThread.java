...
import eu.kalafatic.gemini.bt.tracker.controller.handlers.TrackerClientHandler;
...
/**
 * The Class class ListenPeersThread.
 * @author Petr Kalafatic
 * @version 3.0.0
 * @project Gemini
 */
public class ListenPeersThread extends AServerThread {

	/**
	 * Instantiates a new listen peers thread.
	 */
	public ListenPeersThread() {
		init();
	}
	...
	public void init() {
		try {
			port = PREFERENCES.getInt(ECorePreferences.TRACKER_PORT.getName(), (Integer) ECorePreferences.TRACKER_PORT.getDef());
			message = MessageFormat.format(LISTENING, ETrackerPreferences.MODULE.getName(), "clients", port);
	...
	public void run() {
		try {
			...
			SoundPlayer.getInstance().play("audrey_tracker_is_listenning_for_clients.wav");
			...
			while (enabled) {
				...
				try {
					...
					ThreadPoolExecutor clientExecutor = SwarmThreadsManager.getInstance().getPooledThreads().getClientExecutor();
					clientExecutor.execute(new TrackerClientHandler(session));
